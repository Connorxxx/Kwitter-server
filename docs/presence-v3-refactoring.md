...
Presence 不再经过 `NotificationRepository`/`NotificationEvent`，直接由 `NotificationSse` 在连接生命周期内处理。
...
**新增**：
- `SseConnectionManager.sendToUsers(userIds, event, data)` — 定向多用户推送
...
**SSE 事件格式**，使用 `event` 字段区分类型，`data` 字段为 JSON 负载。
...
id: 2
data: {"users":[{"userId":"u1","isOnline":true,"timestamp":1707600000000},{"userId":"u2","isOnline":false,"timestamp":1707600000000}]}
```
...
id: 3
data: {"userId":"u1","isOnline":true,"timestamp":1707600000000}
```
